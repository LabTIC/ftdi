...
#define JS_PROGRAM_FUNCTION         "program"
#define JS_EEUSERAREA_FUNCTION      "eeuserarea"
...
void FtdiDevice::Initialize(v8::Handle<v8::Object> target) 
{
    ...
    tpl->PrototypeTemplate()->Set(String::NewSymbol(JS_EEUSERAREA_FUNCTION), FunctionTemplate::New(EEUserArea)->GetFunction());
    tpl->PrototypeTemplate()->Set(String::NewSymbol(JS_PROGRAM_FUNCTION), FunctionTemplate::New(Program)->GetFunction());
...
Handle<Value> FtdiDevice::Program(const Arguments& args)
{
    HandleScope scope;
        ...
        if(!args[0]->IsString())
        {
            return scope.Close(v8::ThrowException(Exception::TypeError(String::New("First argument must be a string Manufacturer[64]"))));
        }
        if(!args[1]->IsString())
        {
            return scope.Close(v8::ThrowException(Exception::TypeError(String::New("Second argument must be a string Description[64]"))));
        }
        if(!args[2]->IsString())
        {
            return scope.Close(v8::ThrowException(Exception::TypeError(String::New("Third argument must be a string Serial[64]"))));
        }
        ...
        String::Utf8Value mnf(args[0]->ToString());
        char *manufacturer = *mnf;

        String::Utf8Value dscpt(args[1]->ToString());
        char *description = *dscpt;

        String::Utf8Value cereal(args[2]->ToString());
        char *serial = *cereal;
       ...
       ftStatus = FT_EEPROM_Read(device->ftHandle,&ft_eeprom_232r, sizeof(ft_eeprom_232r), Manufacturer, ManufacturerId, Description, SerialNumber);

       strcpy(SerialNumber, serial);
       strcpy(Manufacturer, manufacturer);
       strcpy(Description, description);

       ftStatus = FT_EEPROM_Program(device->ftHandle,&ft_eeprom_232r, sizeof(ft_eeprom_232r),Manufacturer, ManufacturerId, Description, SerialNumber);

       FT_Close (device->ftHandle);

       return scope.Close(v8::Boolean::New(ftStatus == FT_OK));
}
...
Handle<Value> FtdiDevice::EEUserArea(const Arguments& args)
{
    HandleScope scope;
        ...
        if(!args[0]->IsString())
        {
            return scope.Close(v8::ThrowException(Exception::TypeError(String::New("First argument must be a string [128] depending the size of serial number and others data"))));
        }
        ...
        String::Utf8Value ua(args[0]->ToString());
        char *str = *ua;
        int len = args[0]->ToString()->Length();

        ftStatus = FT_EE_UAWrite(device->ftHandle, (unsigned char*)str, len);
