    ...
    if(listBaton->status == FT_OK)
    {
        ...
        for (DWORD i = 0; i < listBaton->listLength; i++) 
        {
            ...
            if(DeviceMatchesFilterCriteria(&listBaton->devInfo[i], listBaton->vid, listBaton->pid))
            {
                ...
                FT_HANDLE ftHandle;
                FT_STATUS ftStatus = FT_Open(i,&ftHandle);

                if ( ftStatus == FT_OK) {
                    FT_GetComPortNumber(ftHandle,&lComPortNumber);
                    unsigned char buffer[64];
                    DWORD BytesRead;
                    ftStatus = FT_EE_UARead(ftHandle, buffer, 64, &BytesRead);
                    if(ftStatus == FT_OK){
                        obj->Set(String::New(DEVICE_EEPROM), String::New((const char *)buffer, BytesRead));
                    }
                    FT_Close (ftHandle);
                }


                FTID_GetDeviceChipID(i, &ulChipID);



                obj->Set(String::New(DEVICE_PRODUCT_CHIPID), Number::New( ulChipID ));
                obj->Set(String::New(DEVICE_PRODUCT_PortNumber), Number::New( lComPortNumber ));
